...
if __name__ == '__main__':
    ...
    print('*************** Parsing Process Begin *******************')
    print('The input folder is ' + test_folder)
    pp = PedParsing(model_path)
    pp.loadModel()
    print('Successfully load pretrained parsing model: ' + model_path)

    batch_size = 16
    image_name_list = os.listdir(test_folder)
    image_path_list = []
    parsing_all = []
    im_num = len(image_name_list)
    for i in range(im_num):
        image_path_list.append(os.path.join(test_folder, image_name_list[i]))
    ...
    for i in range(im_num):
        ...
        print('Processing: ' + os.path.join(save_folder, image_name))
        save_image_path = os.path.join(save_folder, image_name.replace('.jpg', '.png'))
        cv2.imwrite(save_image_path, parsing_all[i])
    
    print('The parsing results are saved to ' + save_folder)
